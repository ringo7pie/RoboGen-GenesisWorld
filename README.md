...
│   ├── check_env.py              # 環境セルフチェック（--stage genesis|robogen）
│   ├── genesis_record.py         # Genesis カメラ録画ユーティリティ
...
├── requirements/                 # 依存定義（genesis.txt / robogen.txt）
├── configs/colab_paths.yaml      # Colab 上のパス定義
└── docs/                         # アセット手順・トラブルシューティング
```
